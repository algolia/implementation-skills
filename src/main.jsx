...
import './styles.css';
import quickStartFragment from '../public/start/embed.html?raw';
...
// Absolute links baked into the generated quick-start fragment, rewritten to
// stay on the page when it is inlined here.
const SITE_URL = 'https://community.algolia.com/implementation-skills/';
...
// The quick start is generated by enablement/build-customer.py, which also emits
// the standalone page at /start/. Inlining the fragment keeps one source for both;
// its CSS is scoped to .alg so it cannot restyle the site. Scripts are stripped —
// innerHTML never runs them — and the interactions are re-bound below.
const quickStartHtml = quickStartFragment
  .replace(/<script[\s\S]*?<\/script>/g, '')
  .replaceAll(`${SITE_URL}#catalog`, '#catalog')
  .replaceAll(SITE_URL, withBase('/'));
...
function QuickStart() {
  ...
  useEffect(() => {
    ...
    host.querySelectorAll('.block .copy').forEach((button) => {
      const onClick = () => {
        const pre = button.parentElement.querySelector('pre');
        if (!pre) return;
        let text = pre.innerText.replace(/ /g, ' ');
        if (pre.classList.contains('term')) {
          // Terminal blocks show a "$ " prompt marker; strip it so the copy
          // pastes straight into a shell.
    ...
    host.querySelectorAll('.tabs').forEach((group) => {
      const tabs = Array.from(group.querySelectorAll('.tabbar button'));
      ...
      const show = (index, focus) => {
        ...
        try {
          localStorage.setItem('alg-tool', tabs[index].dataset.tool);
        } catch (error) {
          /* private mode — tab choice just won't persist */
        }
      };
      tabs.forEach((tab, i) => {
        const onClick = () => show(i, false);
      ...
      let remembered = null;
      try {
        remembered = localStorage.getItem('alg-tool');
      } catch (error) {
        /* ignore */
      }
      const initial = remembered ? Math.max(0, tabs.findIndex((tab) => tab.dataset.tool === remembered)) : 0;
      show(initial, false);
    });
  ...
  return (
    <section className="quickstart-section" id="start" aria-labelledby="quickstart-title">
      <div className="section-heading">
        <div>
          <h2 id="quickstart-title">Quick start</h2>
          <p>No Algolia experience needed. Download a ZIP, drop it into the AI tool your team already uses, paste one prompt.</p>
        </div>
      </div>
      <div ref={hostRef} dangerouslySetInnerHTML={{ __html: quickStartHtml }} />
    </section>
  );
}
